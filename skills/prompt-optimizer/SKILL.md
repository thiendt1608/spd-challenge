...
1. Check if an `AGENTS.md` exists in the working directory — read it for project conventions
...
|-------|------------------|-----------|
| TRIVIAL-LOW | Fast model tier | Fast, cost-efficient for simple tasks |
| MEDIUM | Balanced coding model | Strong quality-speed balance for standard work |
| HIGH | Balanced model (execution) + high-reasoning model (planning) | Deep planning plus efficient implementation |
| EPIC | High-reasoning model (blueprint) + balanced coding model (execution) | Better architecture quality across long phases |
...
| Model | Balanced coding model | Recommended for this scope |
...
- "Rewrite this prompt so my coding agent uses the right commands"
- "帮我优化这个指令"
...
Recommended: use a high-reasoning model for blueprint planning and a balanced coding model for phase execution.
```
